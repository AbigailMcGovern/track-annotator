...
class SampleViewer:
    def __init__(self, 
        ...
        self.scales = scales
        self.array_order = sample['array_order']
        self.time_col = sample['time_col']
        self.id_col = sample['id_col']
...
def annotate_track_terminations(tracks_path, 
                                image_path, 
                                save_path,
                                labels=None,
                                n_samples=30,
...
def get_log_file_names(
    ...
    if sample_type == 'random tracks':
        suffix = '_random-errors.csv'
    if sample_type == 'terminating tracks':
        suffix = '_terminating-errors.csv'
    if sample_type == 'untracked objects':
        suffix = '_untracked-errors.csv'
    log_path = os.path.join(save_dir, name + suffix)
    sample['log_path'] = log_path
    return sample


def construct_log_file(
    sample,
):
    ...
    cols = cols + ['frames',
                   ...
                   'FN',
                   'seg-error']
    # add additional columns specific to the type of sample
    sample_type = sample['sample_type']
    if sample_type == 'random tracks':
        cols.append('terminating')
        cols.append('track_length')
        cols.append('2-norm')
    if sample_type == 'terminating tracks':
        cols.append('track_length')
        cols.append('2-norm')
    ...
    # if the error log file does not already exist, make a new one
    df_cols = df.columns
    col_selection = []
    for col in cols:
        if col not in df_cols:
            print(f'Sample info does not contain expected column: {col}')
            col_selection.append(col)
    extra_cols = []
    for col in df_cols:
        if col not in cols and not col.startswith('Unnamed:'):
            extra_cols.append(col)
    df_cols = col_selection + extra_cols
    if not os.path.exists(log_path):
        error_log = df[df_cols]
        error_log = error_log.reset_index(drop=True)
        error_log.to_csv(log_path)
